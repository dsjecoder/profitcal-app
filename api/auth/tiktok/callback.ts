export default async function handler(req: any, res: any) {
  ...
  if (error || !code) {
    return res.status(400).send(`
      ...
      <html>
        <head>
          <title>Lỗi ủy quyền TikTok Shop</title>
          <style>
        ...
        <body>
          <div class="card">
            ...
            <p>${error || 'Không nhận được mã ủy quyền từ TikTok Shop Open API.'}</p>
  ...
  return res.status(200).send(`
    ...
    <html>
      <head>
        <title>Ủy quyền TikTok Shop thành công</title>
        <style>
          body { font-family: sans-serif; background: #0f172a; color: #f8fafc; padding: 40px; text-align: center; }
      ...
      <body>
        <div class="card">
          <h2>✓ Đã kết nối gian hàng TikTok Shop</h2>
