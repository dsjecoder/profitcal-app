  ...
  return (
    <div className="bg-white border border-sky-200 rounded-3xl p-6 shadow-2xl space-y-6 my-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-sky-200 pb-5">
        ...
        <div className="flex items-center gap-2 text-xs font-mono text-slate-600 bg-white px-3 py-1.5 rounded-xl border border-sky-200">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-sky-200 rounded-2xl p-4 space-y-2">
          <span className="text-[11px] font-bold text-slate-600 uppercase">Doanh Thu Gộp MoM</span>
          <div className="flex items-baseline justify-between">
            ...
            <span className={`text-xs font-bold flex items-center gap-0.5 px-2 py-0.5 rounded-full ${
              revGrowth.isUp ? 'bg-emerald-500/20 text-emerald-700' : 'bg-rose-500/20 text-rose-700'
            }`}>
        ...
        {/* Thực nhận MoM */}
        <div className="bg-white border border-sky-200 rounded-2xl p-4 space-y-2">
          <span className="text-[11px] font-bold text-slate-600 uppercase">Thực Nhận Ví MoM</span>
          <div className="flex items-baseline justify-between">
            <span className="text-lg font-black font-mono text-indigo-300">{formatVND(summary.netSettlement)}</span>
            <span className={`text-xs font-bold flex items-center gap-0.5 px-2 py-0.5 rounded-full ${
              netGrowth.isUp ? 'bg-emerald-500/20 text-emerald-700' : 'bg-rose-500/20 text-rose-700'
            }`}>
        ...
        {/* Phí sàn MoM */}
        <div className="bg-white border border-sky-200 rounded-2xl p-4 space-y-2">
          <span className="text-[11px] font-bold text-slate-600 uppercase">Phí Sàn Đã Trừ MoM</span>
          <div className="flex items-baseline justify-between">
            <span className="text-lg font-black font-mono text-rose-700">{formatVND(summary.totalFees)}</span>
            <span className={`text-xs font-bold flex items-center gap-0.5 px-2 py-0.5 rounded-full ${
              !feeGrowth.isUp ? 'bg-emerald-500/20 text-emerald-700' : 'bg-amber-500/20 text-amber-700'
            }`}>
        ...
        {/* Lợi Nhuận Ròng MoM */}
        <div className="bg-white border border-emerald-500/30 rounded-2xl p-4 space-y-2">
          ...
          <div className="flex items-baseline justify-between">
            <span className="text-lg font-black font-mono text-emerald-700">{formatVND(summary.netProfit)}</span>
            <span className={`text-xs font-bold flex items-center gap-0.5 px-2 py-0.5 rounded-full ${
              profitGrowth.isUp ? 'bg-emerald-500/20 text-emerald-700' : 'bg-rose-500/20 text-rose-700'
            }`}>
