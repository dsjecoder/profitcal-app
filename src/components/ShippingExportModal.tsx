...
export const ShippingExportModal: React.FC<ShippingExportModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-sky-50/70 backdrop-blur-md animate-fade-in">
      <div className="bg-white border border-sky-200 rounded-3xl w-full max-w-xl p-6 sm:p-8 shadow-2xl relative space-y-6">
        
        {/* Close Button */}
        <button
          type="button"
          onClick={onClose}
          className="absolute top-5 right-5 p-2 rounded-xl bg-white hover:bg-sky-50 text-slate-600 hover:text-white border border-sky-200 transition-all"
        >
          <X className="w-4 h-4" />
        </button>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            ...
            <button
              ...
              className={`p-3 rounded-2xl border text-center transition-all ${
                carrier === 'ghtk'
                  ? 'bg-emerald-500/15 border-emerald-500 text-emerald-700 font-bold shadow-lg shadow-emerald-500/10'
                  : 'bg-white border-sky-200 text-slate-600 hover:text-slate-800'
              }`}
            >
        ...
        {user.tier === 'free' && (
          <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-xs space-y-2">
            ...
            {!cooldownState.canExport && (
              <div className="pt-2 border-t border-amber-500/20 flex items-center justify-between text-amber-700 font-mono font-bold">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-end gap-3 pt-2">
          <button
            type="button"
            onClick={onClose}
            className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-sky-50 hover:bg-slate-100 text-slate-700 font-semibold text-xs transition-colors"
          >
            Đóng
          </button>
