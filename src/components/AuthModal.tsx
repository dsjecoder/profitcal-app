...
export const AuthModal: React.FC<AuthModalProps> = ({ user, onClose, onLoginSuccess }) => {
  ...
  const googleAccounts = [
    { email: 'ecodervn@gmail.com', name: 'Ecodervn Alan Vu', avatarUrl: 'https://lh3.googleusercontent.com/a/ACg8ocI=s96-c' },
    { email: 'dsjecoder@gmail.com', name: 'Dsj Ecoder Vu', avatarUrl: '' },
  ];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      ...
      {/* FLOATING GOOGLE ONE TAP PROMPT ON TOP RIGHT OF MODAL (MATCHING USER SCREENSHOT 100%) */}
      <div className="absolute top-6 right-6 z-50 bg-white text-slate-800 rounded-2xl shadow-2xl border border-slate-200 w-80 p-3.5 text-xs space-y-2 animate-bounce-short">
        <div className="flex items-center justify-between border-b border-slate-100 pb-2">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-700 text-[11px]">Sign in to tagki.com with google.com</span>
          </div>
        ...
        <div className="space-y-1">
          {googleAccounts.map((acc) => (
            <button
              ...
              className="w-full p-2 rounded-xl hover:bg-slate-100 flex items-center gap-3 transition-colors text-left group"
            >
              ...
              <div className="overflow-hidden">
                <div className="font-bold text-slate-800 text-[12px] truncate">{acc.name}</div>
