...
import { Mail, Lock, User, ArrowRight, ShieldCheck, CheckCircle2, X } from 'lucide-react';
...
export const AuthModal: React.FC<AuthModalProps> = ({ user, onClose, onLoginSuccess }) => {
  ...
  const handleGoogleSignIn = () => {
    // Simulate Google Sign In OAuth
    const googleUser = {
      email: 'seller.google@gmail.com',
      name: 'Chủ Shop (Google Account)',
    };
    onLoginSuccess(googleUser.email, googleUser.name);
  };
  ...
  const handleStartRegister = (e: React.FormEvent) => {
    ...
    setStep('verify_otp');
    alert(`[MOCK EMAIL OTP SERVER]: Mã xác nhận kích hoạt tài khoản của bạn là: ${mockOtp}`);
  };
      ...
      <div className="bg-navy-900 border border-navy-700 rounded-3xl w-full max-w-md p-6 sm:p-8 shadow-2xl relative space-y-6">
        ...
        {mode === 'login' ? (
          ...
          <form onSubmit={handleVerifyOtp} className="space-y-4 text-xs animate-fade-in">
            <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 text-center">
              Mã xác thực OTP đã được gửi về email: <strong className="font-mono text-white block">{email}</strong>
            </div>
